...
const fields = [
  defineField({
    ...
    type: 'string',
    description:
      'Choose the position of the image. This will also affect mobile view, so `left` will be on top and `right` will be on bottom',
    options: {
      list: ['left', 'right'],
      layout: 'radio',
...
export const blockTextImage = defineField({
  preview: {
    ...
    prepare({ title, media, text }) {
      if (!title || !media || !text)
        return { title: 'Missing alt tag, media or text' }
